...
/*  03/02/2025      Jayktec  Mejoras procesamiento, comision*/
/*                           por reversos                   */
/************************************************************/
...
BEGIN		
		...
		BEGIN
			...
			BEGIN 	
				...
				BEGIN
					...
					--if @i_op_code = '560050' AND @w_mto_comision > 0 AND @w_cod_error = 0--P2C -- OR @i_op_code='200030') --P2C o C2P 
					if (@i_op_code='560050' OR @i_op_code='560009' OR @i_op_code='200030')  AND @w_mto_comision > 0 AND @w_cod_error = 0
					begin
				...
				END		--Fin No Reverso
				ELSE	
				BEGIN 	--Inicio Reverso
					if @i_op_code='560050' AND @w_mto_comision > 0-- OR @i_op_code='200030') --P2C o C2P 
					begin
						--EJECUTAR NOTA COMISION MONETARIA
						exec @w_return    = @w_rpc
							@s_srv        = @s_srv,
							@s_ofi        = @s_ofi,
							@s_ssn        = @s_ssn,
    ...
    END		--Fin @t_corr <> 'R'
	ELSE 
	BEGIN	--Inicio @t_corr = 'R'
		--Comision minima por transacciones negadas (reversos) a las cuales se les cobro comision	
		IF (@i_op_code='560050' OR @i_op_code='560009' OR @i_op_code='200030' OR @w_socio = 'S') AND @w_cod_error = 0--P2C o C2P 
		BEGIN
			SET @o_sec = @s_ssn_com

			-- Busqueda de parametro de monto minimo
			SELECT @w_mto_com_neg = pa_money
			FROM   cobis..cl_parametro
			WHERE  pa_nemonico = 'MINTTN'

			IF ISNULL(@w_mto_com_neg,0) > 0
			BEGIN
				SET @w_concepto_com_neg = 'COMISION PAGO NEGADO/REVERSO'
